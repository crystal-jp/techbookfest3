...
Crystalの`String`型や`Char`型では，リテラルの中にコントロールコードなどを埋め込むためのエスケープシーケンスが利用可能です。どちらもバックスラッシュ（`\`）に続けて特定の文字列を指定する点では共通していますが，両者が指定できるエスケープシーケンスの種類は微妙に異なっていますので注意が必要です。
